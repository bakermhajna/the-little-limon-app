...
export async function createTable() {
  return new Promise((resolve, reject) => {
    db.transaction(
      (tx) => {
        tx.executeSql(
          'create table if not exists menuitems (id integer primary key not null, uuid text, title text, price text, category text,pic text);',
          [],
 ...
 *
 */
export async function filterByQueryAndCategories(activeCategories,searhinput) {
  return new Promise((resolve) => {
    let conditions = []; // Array to store conditions for each category
    let serachinputquery=""
    ...
    if(searhinput){
      serachinputquery=`AND title LIKE '%${searhinput}%'`
    }
    ...
    db.transaction((tx) => {
      const query=`SELECT * FROM menuitems ${whereClause} ${serachinputquery}`
      console.log(query)
